// Copy paste ke mlpso.cpp jika ingin versi pengujian
...
class MLPSO : public nav_core::BaseGlobalPlanner {
    private:
        ...
        // Parameter untuk PSO
        double c1 = 1.85; // cognitive coefficient
        double c2 = 2.0;  // social coefficient
        double w = 0.65; // inertial weight 
	...
	// Fungsi untuk menyatukan titik awal dan tujuan ke dalam graf
        void mergeSGG(std::vector<Node>& current_graph){
            start_candidates = topKNodes(current_graph, real_start, 8); 
            goal_candidates = topKNodes(current_graph, real_goal, 8);
            ...
            while(currentNode != goalNode_i && steps < max_steps){
                ...
                for(int i = 0; i < n->neighbours_count; ++i){
                    if(!visited[n->neighbours[i].id]){
                        // Heuristic Decoding: Subtract danger from the random priority
	...
	// Fungsi pencarian jalur oleh algoritma pembanding (A*)
        std::vector<int> searchGraph(std::vector<Node>& current_graph, int start_id, int goal_id, double initial_yaw) {
            std::vector<double> g_score(current_graph.size(), 1e9); 
            std::vector<int> came_from(current_graph.size(), -1);
            std::vector<bool> closed_set(current_graph.size(), false);
            std::priority_queue<SearchNode, std::vector<SearchNode>, std::greater<SearchNode>> open_set;

            g_score[start_id] = 0.0;
            open_set.push({start_id, 0.0, initial_yaw}); 
            ...
            while (!open_set.empty()) {
                ...
                if (closed_set[current.id]) continue;
                closed_set[current.id] = true;

                Node& u = current_graph[current.id];
                for (int i = 0; i < u.neighbours_count; ++i) {
                    int neighbor_id = u.neighbours[i].id;
                    if (closed_set[neighbor_id]) continue;

                    double angleToNeighbour = std::atan2(current_graph[neighbor_id].y - u.y, current_graph[neighbor_id].x - u.x);
                    double angleDiff = std::abs(std::atan2(std::sin(angleToNeighbour - current.yaw), std::cos(angleToNeighbour - current.yaw)));

                    double dist_cost = u.neighbours[i].weight * W_DIST;
                    double turn_cost = angleDiff * W_TURN;
                    double edge_cost = u.neighbours[i].penalty * W_EDGE;

                    double tentative_g = g_score[current.id] + dist_cost + turn_cost + edge_cost;

                    if (tentative_g < g_score[neighbor_id]) {
                        came_from[neighbor_id] = current.id;
                        g_score[neighbor_id] = tentative_g;
                        double h = std::hypot(current_graph[neighbor_id].x - current_graph[goal_id].x, current_graph[neighbor_id].y - current_graph[goal_id].y) * W_DIST;
                        open_set.push({neighbor_id, tentative_g + h, angleToNeighbour}); 
                    }
                }
            }
	...
	// Fungsi MLPSO utama 
        bool executePSO(std::vector<Node>& local_graph, int startNode_i, int goalNode_i, int run_id, std::string folder_path, std::vector<int>& final_path, double& final_fitness) {
            ...
            for (int retry = 0; retry < max_retries; ++retry) {
                ...
                std::ofstream pso_log;
                std::string filename = folder_path + "/run_" + std::to_string(run_id) + "_convergence_retry_" + std::to_string(retry + 1) + ".csv";
                pso_log.open(filename);
                pso_log << "Iteration,Time_ms,gBest_Score\n"; 
                
                auto pso_start_time = std::chrono::steady_clock::now();
                ...
                for(int i = 0; i < max_iter; ++i){
                    ...
                    auto elapsed_ms = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(current_time - pso_start_time).count();
                    
                    if (pso_log.is_open()) {
                        pso_log << (i + 1) << "," << elapsed_ms << "," << gBest.g_score << "\n";
                    }
        ...
        // Fungsi untuk mencatat nilai cost akhir serta cost dari setiap objektif
        void evaluatePathCost(const std::vector<int>& path, std::vector<Node>& current_graph, double& total_cost, double& dist_cost, double& turn_cost, double& edge_cost) {
            dist_cost = 0.0; turn_cost = 0.0; edge_cost = 0.0; total_cost = 0.0;
            if (path.empty()) return;

            double currentYaw = start_yaw;

            for (size_t i = 0; i < path.size() - 1; ++i) {
                Node* n1 = &current_graph[path[i]];
                Node* n2 = &current_graph[path[i+1]];

                double weight = 0.0;
                for(int j = 0; j < n1->neighbours_count; ++j){
                    if(n1->neighbours[j].id == n2->id){
                        weight = n1->neighbours[j].weight;
                        break;
                    }
                }

                double angleToNeighbour = std::atan2(n2->y - n1->y, n2->x - n1->x);
                double angleDiff = std::abs(std::atan2(std::sin(angleToNeighbour - currentYaw), std::cos(angleToNeighbour - currentYaw)));
                double tempEdgePenalty = getEdgeCost(n1, n2);

                dist_cost += weight;
                turn_cost += angleDiff;
                edge_cost += tempEdgePenalty;
                currentYaw = angleToNeighbour;
            }

            total_cost = (dist_cost * W_DIST) + (turn_cost * W_TURN) + (edge_cost * W_EDGE);
        }
	...
	// Interpolasi linear dari titik awal ke titik tengah pertama
        void generateSmoothPath(const std::vector<Coordinate>& path_p, std::vector<geometry_msgs::PoseStamped>& plan) {
            ...
            for (int j = 0; j <= steps; ++j){
                ...
                double yaw = std::atan2(mid_y - p0.y, mid_x - p0.x);
            ...
            for (int j = 0; j <= steps; ++j){
                ...
                pose.pose.position.z = 0.0;
                double yaw = std::atan2(p1.y - mid_y, p1.x - mid_x);
                pose.pose.orientation = tf::createQuaternionMsgFromYaw(yaw);
                plan.push_back(pose);
            }
        }
    ...
    public:
        ...
        bool makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan){
            ...
            // Kebutuhan pengambilan data
            static int run_id = 1;
            std::string folder_path = "/tmp/cache_plan";
            if (run_id == 1) {
            ...
            std::string algorithm_mode = "pso"; 

            auto start_time = std::chrono::steady_clock::now();

            // Pencarian jalur 
            if(algorithm_mode == "astar"){
                final_path = searchGraph(local_graph, startNode_i, goalNode_i, start_yaw);
            }
            else if(algorithm_mode == "pso"){
                bool success = executePSO(local_graph, startNode_i, goalNode_i, run_id, folder_path, final_path, final_fitness);
            ...
            ROS_INFO("ALGORITHM            : %s", algorithm_mode.c_str());
            ROS_INFO("COMPUTATIONAL TIME   : %.3f ms", duration);

            // Publikasi node mentah
            ...
            double eval_total = 0.0, eval_dist = 0.0, eval_turn = 0.0, eval_edge = 0.0;
            if(!final_path.empty()){
                evaluatePathCost(final_path, local_graph, eval_total, eval_dist, eval_turn, eval_edge);
            }

            std::string summary_filename = folder_path + "/run_" + std::to_string(run_id) + "_summary_path.csv";
            std::ofstream summary_file(summary_filename);
            
            summary_file << "Run_ID,Total_Time_ms,Final_Fitness,Dist_Cost,Turn_Cost,Edge_Cost,Path_Points_Count\n";
            summary_file << run_id << "," 
                         << duration << "," 
                         << eval_total << "," 
                         << eval_dist << "," 
            ...
            ROS_INFO("Data berhasil disimpan. Folder: %s | Run ID: %d", folder_path.c_str(), run_id);
            run_id++; 
